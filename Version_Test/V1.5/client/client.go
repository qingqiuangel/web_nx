...
func main() {
	...
	for {
		//客户端发送消息
		dp := znet.NewDataPack()
		binaryClientMsg, err := dp.Pack(znet.NewMessage(0, []byte("client message ....")))
		if err != nil {
		...
		if err != nil {
			fmt.Println("client coon.Write failed ,err= ", err)
			return
		}
		...
		if err != nil {
			fmt.Println("client coon.Read failed ,err= ", err)
			return
		}
		//拆包
