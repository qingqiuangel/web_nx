...
func main() {
	...
	for {
		//客户端发送消息
		dp := znet.NewDataPack()
		binaryClientMsg, err := dp.Pack(znet.NewMessage(1, []byte("client1 message ....")))
		if err != nil {
			fmt.Println("client pack msg failed ,err= ", err)
			return
		}
		_, err = coon.Write(binaryClientMsg)
		if err != nil {
			fmt.Println("client1 coon.Write failed ,err= ", err)
			return
		}
		...
		if err != nil {
			fmt.Println("client1 coon.Read failed ,err= ", err)
			return
		}
		//拆包
		headMessage, err := dp.UnPack(headData)
		if err != nil {
			fmt.Println("client dp.UnPack failed ,err= ", err)
			return
		}
		var data []byte
